...
python plot_states.py --gravity-func "exp:9.81,1.1,0.01,0" --x-max 250 --curve
```

### Worked example 1 — field shape moves the descent
...
**falls** it dives early to bank energy while gravity is still strong; a gentle
**ripple** nudges the path toward each local peak of `g`.

Each curve is one solve; the figure overlays them. The per-field commands:

```bash
python brachistochrone.py --gravity-func "const:9.81"        --x-step 5 --y-step 5
python brachistochrone.py --gravity-func "poly:9.81,0.05"    --x-step 5 --y-step 5   # g rises
python brachistochrone.py --gravity-func "poly:9.81,-0.04"   --x-step 5 --y-step 5   # g falls
python brachistochrone.py --gravity-func "sin:3,0.05,0,9.81" --x-step 5 --y-step 5   # g ripples
```

### Worked example 2 — the full path, dipping below the target

![Full paths dipping below the target](docs/images/gravity-dip.png)

Falling **200 m → 100 m** over a long `x = 250 m`, with grid headroom below the
target so the path is not clipped at the endpoint altitude. Because 250 m is
well past the geometric bound (`π·50 ≈ 157` for a 100 m drop), the fastest route
**dips below the 100 m target and climbs back** — the extra depth buys speed that
more than pays for the climb, since the track is frictionless. The decaying field
(green) dives hardest and earliest, cashing in its drop while `g` is still large.

```bash
# one field of the three; --depth-below opens room beneath the target for the dip
python brachistochrone.py --gravity-func "exp:9.81,1.1,0.01,0" \
    --height 200 --end-altitude 100 --x-max 250 \
    --x-step 8 --y-step 8 --depth-below 30 --energy-tol 3
```

`--depth-below 30` gives the search 30 m beneath the target to work in;
`--energy-tol 3` merges near-equal-energy labels, needed because variable
gravity **and** climbing together multiply the label sets (at finer steps the
search trips its own label guard — see [Caveats](#caveats)).

Both figures are regenerated by one script:

```bash
python gravity_examples.py      # writes docs/images/gravity-fields.png and gravity-dip.png
```
...
gravity.py                variable gravity fields: specs, JSON, chord work
make_gravity_json.py      generate a gravity table matching the solver grid
gravity_examples.py       regenerate the two variable-gravity README figures
test_brachistochrone.py   solver tests
test_gravity.py           variable-gravity tests
